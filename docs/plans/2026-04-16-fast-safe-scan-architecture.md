...
- 2026-04-16: Milestone 6 real-machine results:
  ...
  `C:\\Users\\xiongxianfei\\.gradle\\caches` measured `19340 ms` on the first
  run and `5688 ms` on the immediate warm rerun with only `1`
  `describe_path` call; the stable-tree UNC fallback run on
  `\\\\localhost\\D$\\Data\\20260415-space-sift` matched the local fixed
  counts and bytes exactly but took `11706 ms` versus `86 ms` locally and
  required `28786` `describe_path` calls versus `1` locally.
